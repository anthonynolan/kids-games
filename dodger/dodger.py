...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.USEREVENT + 1:
            if selected_player:
                ...
                image = cam.get_image()
                pygame.image.save(
                    image, os.path.join(target_path, f"{time.time()}.jpg")
                )
        ...
        if event.type == pygame.JOYAXISMOTION:
            print(event)
            if (event.axis == 0) & (event.value > 0.1):
                direction = right()
            elif (event.axis == 0) & (event.value < -0.1):
                direction = left()

            elif (event.axis == 1) & (event.value > 0.1):
                direction = down()
            elif (event.axis == 1) & (event.value < -0.1):
                direction = up()
    ...
    predicted_player = (
        ...
        if np.argmax(
            new_model.predict(
                tf.expand_dims(tf.image.resize(image_array, (224, 224)), 0)
            )
        )
        == 0
        else "cathal"
